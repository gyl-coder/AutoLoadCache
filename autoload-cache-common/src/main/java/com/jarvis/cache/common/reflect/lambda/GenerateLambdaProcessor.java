 ...
 *
 * <p>This AbstractProcessor only exists in order to auto-generate source code, namely an interface
 * with multiple signatures. Since the processor is part of the project, where the annotation it
 * processes i located, it is important the compilation proceeds in wel defined rounds, i.e. the
 * processor must be compiled, before the annotated class, which triggers the processor. Being a
 * Maven project, this is currently handled in the pom.xml file, by the maven-compiler-plugin.
 *
...
public class GenerateLambdaProcessor extends AbstractProcessor {
    ...
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        if (!roundEnv.processingOver() && !fileCreated) {
            Set<? extends Element> elements =
                    roundEnv.getElementsAnnotatedWith(GenerateLambda.class);
            for (Element element : elements) {
                if (element.getKind() == ElementKind.CLASS) {
                    GenerateLambda generateSignatureContainerAnnotation =
                            element.getAnnotation(GenerateLambda.class);
    ...
    private void generateCode(
        ...
        try (Writer writer = filer.createSourceFile(fqcn).openWriter()) {
            ...
            javaFile.append(
                    ...
                    .append("\n */")
                    .append(
                            "\n@javax.annotation.Generated(value=\"com.hervian.lambda.GenerateLambdaProcessor\", date=\"")
                    .append(new Date())
                    .append("\")")
            ...
            throw new RuntimeException(
                    "An exception occurred while generating the source file " + METHOD_NAME, e);
        }
    }

    private void generateAbstractandConcreteMethods(
            StringBuilder javaFile, GenerateLambda generateSignatureContainerAnnotation) {
    ...
    private void generateAbstractMethods(
            StringBuilder javaFile, MethodParameter[] types, int maxNumberOfParams) {
        List<String> returnTypes =
                Arrays.asList(types).stream()
                        .map(type -> type.getTypeAsSourceCodeString())
                        .collect(Collectors.toList());
        returnTypes.add("void");
        ICombinatoricsVector<MethodParameter> originalVector =
                CombinatoricsFactory.createVector(types);
        generateInterfaceMethodsForStaticCallsWithMaxNumOfArgs(
                javaFile, originalVector, returnTypes, maxNumberOfParams + 1);
        generateInterfaceMethodCombinationsRecursively(
                javaFile, originalVector, returnTypes, maxNumberOfParams);
    }

    private void generateInterfaceMethodsForStaticCallsWithMaxNumOfArgs(
            StringBuilder javaFile,
            ICombinatoricsVector<MethodParameter> originalVector,
            List<String> returnTypes,
            int numberOfParams) {
        Generator<MethodParameter> gen =
                CombinatoricsFactory.createPermutationWithRepetitionGenerator(
                        originalVector, numberOfParams);
            ...
            for (ICombinatoricsVector<MethodParameter> paramType : gen) {
                if (paramType.getVector().get(0) == MethodParameter.OBJECT) {
                    ...
                    javaFile.append(NEWLINE_TAB)
                            .append(returnTypeAsString)
                            .append(getSignatureExclArgsAndReturn(returnTypeAsString))
                            .append(parameters)
                            .append(END_OF_SIGNATURE);
                }
            }
        }
    }

    private void generateInterfaceMethodCombinationsRecursively(
            StringBuilder javaFile,
            ICombinatoricsVector<MethodParameter> originalVector,
            List<String> returnTypes,
            int numberOfParams) {
        if (numberOfParams >= 0) {
            ...
            generateInterfaceMethodCombinationsRecursively(
                    javaFile, originalVector, returnTypes, --numberOfParams);
        }
    }

    private void generateInterfaceMethods(
            Generator<MethodParameter> gen, String returnTypeAsString, StringBuilder javaFile) {
        javaFile.append(NEWLINE);
        for (ICombinatoricsVector<MethodParameter> paramType : gen) {
            String parameters = getParametersString(paramType, javaFile);
            javaFile.append(NEWLINE_TAB)
    ...
    private String getParametersString(
            ICombinatoricsVector<MethodParameter> paramType, StringBuilder javaFile) {
